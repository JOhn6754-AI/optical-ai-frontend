    ...
    <div className="job-row" style={{ borderLeft: `3px solid ${color}` }}>
      <div className="job-row-main">
        ...
        <div className="job-stats">
          ...
          <span className="job-rate" style={{ color }}>${job.gross_per_hour}/hr</span>
      ...
      <div className="job-row-detail">
        {job.duration_hours}hr work · {job.drive_time}hr drive ·
        ${job.labor_cost} labor cost · ${job.profit > 0 ? "+" : ""}{job.profit} profit
      </div>
    </div>
  );
}
  ...
  return (
    <div className="dashboard">
      <div className="dash-header">
        <div>
          {businessName && <div className="dash-biz-name">{businessName}</div>}
          <h2 className="section-title">Weekly Analysis</h2>
        </div>
        <div className="dash-header-right">
          {data.geocoding === "real" && (
            <div className="geocoding-badge">✓ Real drive times</div>
          )}
          <button className="reset-btn" onClick={onReset}>↑ New Upload</button>
        </div>
      </div>
        ...
        {days.map((day) => (
          <div key={day.date} className="day-block">
            ...
            <div className="jobs-list">
              {day.jobs.map((job, i) => <JobRow key={i} job={job} />)}
            </div>
          </div>
        ))}
      </section>
