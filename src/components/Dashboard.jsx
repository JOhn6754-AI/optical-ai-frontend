...
function JobRow({ job, onClick }) {
  const color = COLORS[job.classification];
  return (
    <div onClick={() => onClick(job)} style={{ borderLeft: `3px solid ${color}`, background: "#0f0f0f", borderRadius: "0 8px 8px 0", padding: "14px 16px", marginBottom: 6, cursor: "pointer", transition: "background 0.15s" }}
      ...
      <div style={{ fontSize: 12, color: "#6b7280" }}>
        {job.duration_hours}hr work · {job.drive_time}hr drive · ${job.labor_cost} labor · <span style={{ color: job.profit >= 0 ? "#22c55e" : "#ef4444" }}>{job.profit >= 0 ? "+" : ""}${job.profit} profit</span>
      </div>
    </div>
  );
}
      ...
      <div style={{ background: "#1a1f2e", border: "1px solid #6366f133", borderRadius: 12, padding: "20px 24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: 12, color: "#6b7280" }}>{optimization.total_hours_saved}hr saved · {optimization.pct_improvement}% improvement</div>
          </div>
        </div>
        ...
        <button onClick={() => setExpanded(!expanded)} style={{ background: "none", border: "none", color: "#6366f1", fontSize: 13, cursor: "pointer", padding: 0, display: "flex", alignItems: "center", gap: 4 }}>
            ...
            <CircleMarker key={i} center={[job.lat, job.lng]} radius={9} pathOptions={{ fillColor: COLORS[job.classification], color: "#000", fillOpacity: 0.85, weight: 1.5 }}>
              <Popup>
                <div style={{ minWidth: 160 }}>
                  ...
                  <span style={{ fontSize: 13 }}>${job.revenue} · </span>
      ...
      <div style={{ display: "flex", gap: 18, marginTop: 8, fontSize: 12, color: "#6b7280" }}>
        {Object.entries(LABELS).map(([cls, label]) => (
          <span key={cls} style={{ display: "flex", alignItems: "center", gap: 5 }}>
            <span style={{ width: 10, height: 10, borderRadius: "50%", background: COLORS[cls], display: "inline-block", flexShrink: 0 }} />
            {label}
          </span>
        ))}
      </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 10 }}>
            {red_jobs.map((job, i) => (
              <div key={i} style={{ background: "#ef444410", border: "1px solid #ef444430", borderRadius: 10, padding: "14px 16px" }}>
                ...
                <div style={{ fontSize: 13, color: "#fca5a5", marginBottom: 8 }}>${job.revenue} quoted · <span style={{ color: "#ef4444" }}>${job.gross_per_hour}/hr effective</span></div>
